...
stm_lms_register_script('account-statistics', [], false, " Vue.http.options.root = '" . get_site_url() . STM_LMS_BASE_API_URL . "'; var account_statistics_data = JSON.parse('" . stm_lms_convert_content(json_encode($data)) . "');");
...
<div id="stm-account-statistics" v-bind:class="{'min-height-500': order_items.length}">
    <div class="stm-lms-user-quizzes">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-md-5 col-sm-6">
                        <button @click="save_email" class="btn btn-default"
                            ...
                            <span><?php esc_html_e('Save', 'masterstudy-lms-learning-management-system'); ?></span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div v-if="order_items.length" class="stm-lms-user-quiz__head heading_font">
            ...
            <div class="stm-lms-user-quiz__head_title text-right">
                <h5 class="p-t-15"><?php esc_html_e('Total price', 'masterstudy-lms-learning-management-system'); ?> :
                    <strong>{{total_price}}</strong></h5>
            </div>
            <div class="stm-lms-user-quiz__head_title text-right">
                <h5 class="p-t-15"><?php esc_html_e('Total count', 'masterstudy-lms-learning-management-system'); ?> :
    ...
    <table class="table table-striped" v-if="order_items.length">
        ...
        <tr>
            ...
            <th><?php esc_html_e('Total Price', 'masterstudy-lms-learning-management-system'); ?>
                ({{currency_symbol}})
            </th>
    ...
    <div v-if="order_items.length" class="text-center p-b-30">
        <button v-if="show_load_more" @click="load_more()" class="btn btn-default"
            ...
            <span><?php esc_html_e('Load more', 'masterstudy-lms-learning-management-system'); ?></span></button>
    </div>
    <h3 class="text-center p-t-30"
        v-if="!order_items.length"><?php _e("Statistics have not yet appeared.", "masterstudy-lms-learning-management-system") ?></h3>
</div>
