        ...
        new Vue({
            ...
            methods: {
                getUsers() {
                    ...
                    var url = stm_lms_ajaxurl + '?action=stm_lms_get_point_users&nonce='
                        + stm_lms_nonces['stm_lms_get_point_users'] +
                        '&page=' + vm.page +
                        '&s=' + vm.search;
                ...
                loadHistory(user, page) {
                    var vm = this;

                    vm.$set(user, 'loading', true);
                    vm.$set(user, 'current_page', page);

                    var url = stm_lms_ajaxurl + '?action=stm_lms_get_user_points_history_admin&nonce='
                        + stm_lms_nonces['stm_lms_get_user_points_history_admin'] +
                        '&page=' + page +
                        '&user_id=' + user['ID'];
                ...
                changePoints(point, user) {
                    ...
                    if (typeof point.edit === 'string') {
                        ...
                        var url = stm_lms_ajaxurl + '?action=stm_lms_change_points&nonce='
                            + stm_lms_nonces['stm_lms_change_points'] +
                            '&point=' + current_point +
                            '&user_points_id=' + point['user_points_id'] +
                            '&user_id=' + point['user_id'] +
                            '&prev_point=' + point['score'];
                ...
                deletePoint(point, pointIndex, user) {
                    if (!confirm(this.translations['delete_action'])) return false;

                    var vm = this;
                    vm.$set(point, 'loading', true);

                    var url = stm_lms_ajaxurl + '?action=stm_lms_delete_points&nonce='
                        + stm_lms_nonces['stm_lms_delete_points'] +
                        '&user_points_id=' + point['user_points_id'] +
                        '&user_id=' + point['user_id'];
