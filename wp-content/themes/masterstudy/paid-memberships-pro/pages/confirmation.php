...
<?php if(!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>		
	...
	<ul>
		...
		<li><strong><?php _e('Account', 'masterstudy' );?>:</strong> <?php echo sanitize_text_field($current_user->display_name); ?> (<?php echo sanitize_text_field($current_user->user_email); ?>)</li>
		<li><strong><?php _e('Membership Level', 'masterstudy' );?>:</strong> <?php echo sanitize_text_field($current_user->membership_level->name); ?></li>
		<?php if($current_user->membership_level->enddate) { ?>
			<li><strong><?php _e('Membership Expires', 'masterstudy' );?>:</strong> <?php echo date_i18n(get_option('date_format'), $current_user->membership_level->enddate)?></li>
		<?php } ?>
		<?php if($pmpro_invoice->getDiscountCode()) { ?>
			<li><strong><?php _e('Discount Code', 'masterstudy' );?>:</strong> <?php echo sanitize_text_field($pmpro_invoice->discount_code->code); ?></li>
		<?php } ?>
	...
	<div class="pmpro_invoice_details">
		<?php if(!empty($pmpro_invoice->billing->name)) { ?>
			<div class="pmpro_invoice-billing-address">
				...
				<?php if($pmpro_invoice->billing->city && $pmpro_invoice->billing->state) { ?>
					<?php echo sanitize_text_field($pmpro_invoice->billing->city); ?>, <?php echo sanitize_text_field($pmpro_invoice->billing->state); ?> <?php echo sanitize_text_field($pmpro_invoice->billing->zip); ?> <?php echo sanitize_text_field($pmpro_invoice->billing->country); ?><br />
				<?php } ?>
		...
		<?php if($pmpro_invoice->accountnumber) { ?>
			<div class="pmpro_invoice-payment-method">
				...
				<p><?php _e('Expiration', 'masterstudy' );?>: <?php echo sanitize_text_field($pmpro_invoice->expirationmonth); ?>/<?php echo sanitize_text_field($pmpro_invoice->expirationyear); ?></p>
		...
		<div class="pmpro_invoice-total">
			...
			<p><?php if($pmpro_invoice->total != '0.00') { ?>
				<?php if(!empty($pmpro_invoice->tax)) { ?>
					<?php _e('Subtotal', 'masterstudy' );?>: <?php echo pmpro_formatPrice($pmpro_invoice->subtotal);?><br />
					<?php _e('Tax', 'masterstudy' );?>: <?php echo pmpro_formatPrice($pmpro_invoice->tax);?><br />
					<?php if(!empty($pmpro_invoice->couponamount)) { ?>
						<?php _e('Coupon', 'masterstudy' );?>: (<?php echo pmpro_formatPrice($pmpro_invoice->couponamount);?>)<br />
					<?php } ?>
					<strong><?php _e('Total', 'masterstudy' );?>: <?php echo pmpro_formatPrice($pmpro_invoice->total);?></strong>
				<?php } else { ?>
...
<?php 
	...
	<ul>
		<li>
            ...
            <?php echo sanitize_text_field($current_user->display_name); ?> (<?php echo sanitize_text_field($current_user->user_email); ?>)
        </li>
...
<nav id="nav-below" class="navigation" role="navigation">
	<div class="nav-next">
		<?php if(!empty($current_user->membership_level)) { ?>
			<a href="<?php echo esc_url(STM_LMS_User::user_page_url()); ?>" class="btn btn-default"><?php _e('View Your Account', 'masterstudy' );?></a>
		<?php } else { ?>
